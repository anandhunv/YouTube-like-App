...
import { Logo, USER_ICON, YOUTUBE_SEARCH_API } from '../utils/constants';
...
const Header = () => {
  ...
  return (
    <div className='md:p-4 md:mb-0 mb-4 md:grid md:grid-flow-col flex flex-col md:mt-0 px-2'>
      <div className='left md:flex md:col-span-2 flex md:flex-row flex-row-reverse p-3 md:p-0 justify-between md:justify-normal'>
        <i className='fa-solid fa-bars text-[24px] md:text-[20px] md:mr-8 text-white cursor-pointer' onClick={toggleMenuHandler}></i>
        <Link to='/'>
      ...
      <div className='middle bg-neutral-600 md:flex flex md:mx-0 mx-3 md:items-start rounded-2xl md:h-[45px] mt-4 md:mt-0 md:col-span-6 col-span-6 h-[40px] md:justify-end'>
        ...
        />
        <div className='absolute md:mr-14 md:ml-72 ml-64 md:mt-1 mt-2 md:text-2xl font-extralight'>{searchQuery && <h1 className='cursor-pointer px-1 hover:border border-zinc-900' onClick={handleClearSearch}>X</h1>}</div>
      ...
      <div className='fixed flex rounded-b-none bg-black md:w-[650px] md:h-auto w-screen md:p-0 p-5 md:mx-72 rounded-lg md:pt-3 mt-[110px] md:mt-[47px]'>
      ...
      <div className='right md:flex md:col-span-2 md:px-5 md:justify-end md:relative flex md:ml-0 ml-[300px] md:mt-0 mt-3 absolute'>
          <div className='md:flex flex  md:border md:border-solid md:border-neutral-700 md:px-1 md:mt-0 mt-[2px] md:py-1 -ml-4 md:justify-center md:items-center'>
           <img src={USER_ICON} alt='' className='md:h-8 h-5 cursor-pointer' title='Enjoy As Guest'/>
          </div>
      </div>
    </div>
  );
